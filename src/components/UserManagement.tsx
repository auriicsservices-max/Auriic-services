...
import { UserPlus, Shield, User as UserIcon, Trash2, Mail, Lock, Loader2 } from 'lucide-react';
...
export default function UserManagement() {
  ...
  const handleDeleteUser = async (userId: string) => {
    if (!window.confirm('Are you sure you want to delete this user? This does not delete their auth account from Firebase console, but removes their access to the portal.')) return;
    try {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <section className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-serif text-slate-800">Active Team Registry</h3>
          ...
          {users.map((u) => (
            <div key={u.id} className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 group transition-all hover:bg-white hover:border-slate-200 hover:shadow-sm">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-sm font-bold text-indigo-600 shadow-sm group-hover:bg-indigo-600 group-hover:text-white transition-all">
